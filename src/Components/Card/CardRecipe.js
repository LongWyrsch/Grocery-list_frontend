...
export const CardRecipe = ({
	...
	const updatefield = (index, col, newValue) => {
		...
		// Fetching nutrition API on every key stroke is too slow. I did not manage to make this async.
		// For now, nutrition API is only fetch when getting recipes from database.
		// let newkcal = getKcal(row.ingredient, row.quantity, row.unit)
		setFocusCard((prev) => {
	...
	return (
		<div className={`card-elevated  ${styles.cardWrapper}`} onClick={stopPropagation}>
			...
			<div className={styles.colHeaders}>
				<h4 className={`generalText ${styles.handleHeader}`}> </h4>

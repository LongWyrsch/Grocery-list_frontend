...
export const CornerAvatar = () => {
	...
	useEffect(() => {
		let handler = (e) => {if (!menuRef.current.contains(e.target)) setOpen(false)};
	...
	return (
		<div className={styles.dropdownContainer}>
			<button onClick={openMenu} className={styles.avatar}>
				<Avatar
					size={40}
					name={user.email || user.googleName}
			...
			<div className={`${open ? styles.activeMenu : styles.inactiveMenu} ${styles.menu}`} ref={menuRef}>
				...
				<LanguageOption option="Settings" icon={gearIcon} handleClick={() => { navigate('/account') }} />
				<hr/>
